...
public class ProfileWishlist extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        GameButtonFactory.getWishedGames(mViewModel);
        mViewModel.wishedGames.observe(getViewLifecycleOwner(), gameModels -> {
            if (!gameModels.isEmpty()) {
                recyclerView = getActivity().findViewById(R.id.wishlist_recyclerview);

                GroupAdapter adapter = new GroupAdapter();

                for (GameModel game : Objects.requireNonNull(mViewModel.wishedGames.getValue())) {
